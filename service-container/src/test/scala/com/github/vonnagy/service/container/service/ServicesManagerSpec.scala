...
class ServicesManagerSpec extends AkkaTestkitSpecs2Support(ActorSystem("test", {
  ...
  "The ServicesManager" should {
    ...
    "be able to find a registered service by name" in {
      ...
      val mgrProps = Props {
        val spied = spy(new ServicesManager(containerService, Nil, props))
        doNothing.when(spied).initializeHttpServer()
        spied
      }

      val act = TestActorRef[ServicesManager](mgrProps, "service3")
      act.underlying.become(act.underlyingActor.running)
    ...
    "be able to find a registered service by name using the companion object" in {
      ...
      val act = TestActorRef[ServicesManager](mgrProps, "service4")
      act.underlying.become(act.underlyingActor.running)
    ...
    "return a failure when using the wrong service manager path" in {
      val mgrProps = Props {
        val spied = spy(new ServicesManager(containerService, Nil, Nil))
        doNothing.when(spied).initializeHttpServer()
        spied
      }

      val act = TestActorRef[ServicesManager](mgrProps, "service5")
      act.underlying.become(act.underlyingActor.running)
    ...
    "return a failure when a service can't be found" in {
      ...
      val act = TestActorRef[ServicesManager](mgrProps, "service6")
      act.underlying.become(act.underlyingActor.running)
